...
if (Req::val('user_name') != '' && Req::val('password') != '') {
    ...
    if (($user_id = Flyspray::checkLogin($username, $password)) < 1) {
		...
		// setCookie($name, $val, $time = null, $secure = false, $httponly = false)
        Flyspray::setcookie('flyspray_userid', $user->id, $cookie_time, true);
        Flyspray::setcookie('flyspray_passhash', $passweirded, $cookie_time, true);
