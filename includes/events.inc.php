...
function get_events($task_id, $where = '')
{
    ...
    return $db->Query("SELECT h.*,
            ...
            LEFT JOIN {list_tasktype} tt1 ON cast(tt1.tasktype_id as varchar) = h.old_value AND h.field_changed='task_type'
            LEFT JOIN {list_tasktype} tt2 ON cast(tt2.tasktype_id as varchar) = h.new_value AND h.field_changed='task_type'

            LEFT JOIN {list_os} los1 ON cast(los1.os_id as varchar) = h.old_value AND h.field_changed='operating_system'
            LEFT JOIN {list_os} los2 ON cast(los2.os_id as varchar) = h.new_value AND h.field_changed='operating_system'

            LEFT JOIN {list_category} lc1 ON cast(lc1.category_id as varchar) = h.old_value AND h.field_changed='product_category'
            LEFT JOIN {list_category} lc2 ON cast(lc2.category_id as varchar) = h.new_value AND h.field_changed='product_category'

            LEFT JOIN {list_status} ls1 ON cast(ls1.status_id as varchar) = h.old_value AND h.field_changed='item_status'
            LEFT JOIN {list_status} ls2 ON cast(ls2.status_id as varchar) = h.new_value AND h.field_changed='item_status'

            LEFT JOIN {list_resolution} lr ON cast(lr.resolution_id as varchar) = h.new_value AND h.event_type = 2

            LEFT JOIN {projects} p1 ON cast(p1.project_id as varchar) = h.old_value AND h.field_changed='project_id'
            LEFT JOIN {projects} p2 ON cast(p2.project_id as varchar) = h.new_value AND h.field_changed='project_id'

            LEFT JOIN {comments} c ON cast(c.comment_id as varchar) = h.field_changed AND h.event_type = 5

            LEFT JOIN {attachments} att ON cast(att.attachment_id as varchar) = h.new_value AND h.event_type = 7

            LEFT JOIN {list_version} lv1 ON cast(lv1.version_id as varchar) = h.old_value
                      AND (h.field_changed='product_version' OR h.field_changed='closedby_version')
            LEFT JOIN {list_version} lv2 ON cast(lv2.version_id as varchar) = h.new_value
                      AND (h.field_changed='product_version' OR h.field_changed='closedby_version')
