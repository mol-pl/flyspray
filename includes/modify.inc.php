 ...
 * @version  $Id: modify.inc.php 1423 2007-08-22 10:06:57Z floele $
 */
...
switch ($action = Req::val('action'))
{
    ...
    case 'details.update':
		...
		//
		// by Nux: don't allow to change some...
		$sql_fileds['set'] = '
			project_id = ?, task_type = ?,
			item_summary = ?, detailed_desc = ?,
			item_status = ?, mark_private = ?,
			product_category = ?, 
			operating_system = ?, task_severity = ?,
			task_priority = ?, last_edited_by = ?, last_edited_time = ?, due_date = ?,
			percent_complete = ?, product_version = ?
		';
		$sql_fileds['vals'] = array(
			Post::val('project_id'), Post::val('task_type'),
		...
		// check some perms
		if (Post::val('task_type')!=2 || $user->perms('modify_all_tasks'))	// if (not feature OR perm to modify all tasks)
		{
			$sql_fileds['set'] .= ', closedby_version = ?';
			$sql_fileds['vals'][] = Post::val('closedby_version', 0);
		}
		// by END
		//
		
		// Nux start - anon_email change
		if (Post::val('anon_email_is_to_be_mod', 0))
		{
			$sql_fileds['set'] .= ', anon_email = ?';
			$sql_fileds['vals'][] = Post::val('anon_email', '');
		}
		// Nux end
    ...
    // ##################
        ...
        $db->Query("UPDATE  {tasks}
                     ...
                     WHERE  task_id = ?",
                    array(time(), $user->id, empty($old_percent['old_value']) ? 0 : $old_percent['old_value'], $task['task_id']));
    ...
    case 'details.addcomment':
        ...
        if (!Backend::add_comment($task, Post::val('comment_text'), $time)) {
            Flyspray::show_error(L('nocommententered'));
            break;
        }

		//
		// by Nux: Allow status change upon adding comments
		Backend::incomment_task_edit($task, $_POST, $time);
		// by END
		//
    ...
    case 'editcomment':
		...
		// Nux: notify of change about the comment
        if (Post::val('comment_text') != Post::val('previous_text')) {
            $notify->Create(NOTIFY_TASK_CHANGED, $task['task_id'], array());
        }

		// Nux: Allow status change upon changing comments
		Backend::incomment_task_edit($task, $_POST, $time);
    ...
    // ##################
    case 'deletereminder':
        if ( ( !$user->perms('edit_assignments') && !$user->perms('manage_project') ) || !is_array(Post::val('reminder_id'))) {
            break;
        }
