...
/*
	Export to Gantt
*/
function jsGanntExportClick(form)
{
	// ids
	var ids = form['ids[]'];
	var ids_checked = [];
	for (var i=0; i<ids.length; i++)
	{
		if (ids[i].checked)
		{
			ids_checked[ids_checked.length] = ids[i].value;
		}
	}
	// base date
	var gantt_base_date = '';
	try
	{
		gantt_base_date = document.getElementsByName('gantt_base_date')[0].value;
	}
	catch(e) {}
	// only when not empty
	//if (ids_checked.length)
	{
		var lnk = location.href.replace(/\??.*/, '?do=gantt_export');
		lnk += "&ids="+ids_checked.join(',');
		lnk += "&gantt_base_date="+gantt_base_date;
		location.href = lnk;
	}
}
