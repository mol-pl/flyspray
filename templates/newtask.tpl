...
<div id="taskdetails">
    ...
    <div id="taskfields">
      <table>
        ...
        <tr>
		  <?php
			// domyślnie zaznaczona wersja podana w paramterach...
			if (Req::val('product_version')!=NULL)
			{
				$ver_default = Req::val('product_version');
			}
			// ...albo pierwsza z teraźniejszych
			else
			{
				$biezaca = $proj->listVersions(false, '2');
				$ver_default = $biezaca[0]['version_id'];	//version_name
			}
		  ?>
          <td><label for="reportedver">{L('reportedversion')}</label></td>
          <td>
            <select class="adminlist" name="product_version" id="reportedver">
              {!tpl_options($proj->listVersions(false, $conf['general']['new_task_reportedver_tenses']), $ver_default)}
            </select>
          </td>
        </tr>
        <tr>
          ...
          <td>
            <select id="dueversion" name="closedby_version" {!tpl_disableif(!$user->perms('modify_all_tasks'))}>
              ...
              {!tpl_options($proj->listVersions(false, '2,3'), Req::val('closedby_version'))}
            </select>
          </td>
        </tr>
        <?php if ($user->perms('modify_all_tasks')): ?>
    ...
    <div id="taskdetailsfull">
      ...
      <?php endif; ?>
      {!TextFormatter::textarea('detailed_desc', 30, 70, array('id' => 'details'), strip_tags(Req::val('detailed_desc', $proj->prefs['default_task'])))}
