...
class DokuWiki_Syntax_Plugin extends Doku_Parser_Mode {
    ...
    var $dokuConf = array();            // array to hold plugin settings, best accessed via ->getConf()
    ...
    function localFN($id) {
      ...
      $file = DOKU_PLUGIN.$plugin.'/lang/'.$dokuConf['lang'].'/'.$id.'.txt';
      if(!@file_exists($file)){
    ...
    function setupLocale() {
      ...
      global $dokuConf;            // definitely don't invoke "global $lang"
      $path = DOKU_PLUGIN.$this->getPluginName().'/lang/';
      
      // don't include once, in case several plugin components require the same language file
      @include($path.'en/lang.php');    
      if ($dokuConf['lang'] != 'en') @include($path.$dokuConf['lang'].'/lang.php');
  ...
  function loadConfig(){
    ...
    foreach ($defaults as $key => $value) {
      if (isset($dokuConf['plugin'][$plugin][$key])) continue;
      $dokuConf['plugin'][$plugin][$key] = $value;
    }

    $this->configloaded = true;
    $this->conf =& $dokuConf['plugin'][$plugin];    
  }
