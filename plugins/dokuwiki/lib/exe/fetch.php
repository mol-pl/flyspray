...
function sendFile($file,$mime,$cache){
  ...
  if ($cache==-1) {
    ...
    header('Expires: '.gmdate("D, d M Y H:i:s", time()+max($dokuConf['cachetime'], 3600)).' GMT');
    header('Cache-Control: public, proxy-revalidate, no-transform, max-age='.max($dokuConf['cachetime'], 3600));
    header('Pragma: public');
  } else if ($cache>0) {
    // recache
    // remaining cachetime + 10 seconds so the newly recached media is used
    header('Expires: '.gmdate("D, d M Y H:i:s", $fmtime+$dokuConf['cachetime']+10).' GMT');
    header('Cache-Control: public, proxy-revalidate, no-transform, max-age='.max($fmtime-time()+$dokuConf['cachetime']+10, 0));
    header('Pragma: public');
  } else if ($cache==0) {
    // nocache
...
function calc_cache($cache){
  ...
  if(strtolower($cache) == 'recache') return $dokuConf['cachetime']; //use standard cache
...
function get_from_URL($url,$ext,$cache){
  ...
  if (!$dokuConf['fetchsize']) return false;
...
function image_download($url,$file){
  ...
  $http->max_bodysize = $dokuConf['fetchsize'];
  ...
  fwrite($fp,$data);
  fclose($fp);
  if(!$fileexists and $dokuConf['fperm']) chmod($file, $dokuConf['fperm']);
...
function resize_imageIM($ext,$from,$from_w,$from_h,$to,$to_w,$to_h){
  ...
  if(!$dokuConf['im_convert']) return false;

  // prepare command
  $cmd  = $dokuConf['im_convert'];
  $cmd .= ' -resize '.$to_w.'x'.$to_h.'!';
  if ($ext == 'jpg' || $ext == 'jpeg') {
      $cmd .= ' -quality '.$dokuConf['jpg_quality'];
  }
  $cmd .= " $from $to";
...
function resize_imageGD($ext,$from,$from_w,$from_h,$to,$to_w,$to_h){
  ...
  if($dokuConf['gdlib'] < 1) return false; //no GDlib available or wanted
  ...
  if(($dokuConf['gdlib']>1) && function_exists("imagecreatetruecolor")){
  ...
  if($ext == 'png' && $dokuConf['gdlib']>1 && function_exists('imagesavealpha')){
  ...
  if ($ext == 'jpg' || $ext == 'jpeg'){
    if(!function_exists('imagejpeg')){
      ...
      $okay = imagejpeg($newimg, $to, $dokuConf['jpg_quality']);
    }
  }elseif($ext == 'png') {
