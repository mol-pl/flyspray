 ...
 *
 * For $param='id' $dokuConf['start'] is returned if no id was found.
...
function getID($param='id',$clean=true){
  ...
  if(empty($id) && $dokuConf['userewrite'] == 2){
    //get the script URL
    if($dokuConf['basedir']){
      $relpath = '';
      if($param != 'id') {
      ...
      $script = $dokuConf['basedir'].$relpath.basename($_SERVER['SCRIPT_FILENAME']);
  ...
  if(empty($id) && $param=='id') $id = $dokuConf['start'];
...
function cleanID($raw_id,$ascii=false){
  ...
  $id = strtr($id,';',':');
  if($dokuConf['useslash']){
    $id = strtr($id,'/',':');
  }else{
    $id = strtr($id,'/',$sepchar);
  }

  if($dokuConf['deaccent'] == 2 || $ascii) $id = utf8_romanize($id);
  if($dokuConf['deaccent'] || $ascii) $id = utf8_deaccent($id,-1);
...
function wikiFN($raw_id,$rev='',$clean=true){
  ...
  $id = str_replace(':','/',$id);
  if(empty($rev)){
    $fn = $dokuConf['datadir'].'/'.utf8_encodeFN($id).'.txt';
  }else{
    $fn = $dokuConf['olddir'].'/'.utf8_encodeFN($id).'.'.$rev.'.txt';
    if($dokuConf['compression']){
...
function wikiLockFN($id) {
  global $dokuConf;
  return $dokuConf['lockdir'].'/'.md5(cleanID($id)).'.lock';
}
...
function metaFN($id,$ext){
  ...
  $id = cleanID($id);
  $id = str_replace(':','/',$id);
  $fn = $dokuConf['metadir'].'/'.utf8_encodeFN($id).$ext;
  return $fn;
}
...
function mediaFN($id){
  ...
  $id = cleanID($id);
  $id = str_replace(':','/',$id);
    $fn = $dokuConf['mediadir'].'/'.utf8_encodeFN($id);
  return $fn;
}
...
function localeFN($id){
  global $dokuConf;
  $file = DOKU_INC.'inc/lang/'.$dokuConf['lang'].'/'.$id.'.txt';
  if(!@file_exists($file)){
...
function resolve_pageid($ns,&$page,&$exists){
  ...
  if(substr($page,-1) == ':'){
    if(@file_exists(wikiFN($page.$dokuConf['start']))){
      // start page inside namespace
      $page = $page.$dokuConf['start'];
      $exists = true;
...
/**
 * Checks a pageid against $dokuConf['hidepages']
 *
 * @author Andreas Gohr <gohr@cosmocode.de>
 */
function isHiddenPage($id){
  global $dokuConf;
  if(empty($dokuConf['hidepages'])) return false;

  if(preg_match('/'.$dokuConf['hidepages'].'/ui',':'.$id)){
    return true;
  }
  return false;
}
