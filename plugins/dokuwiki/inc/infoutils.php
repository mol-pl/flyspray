...
function checkUpdateMessages(){
    global $dokuConf;
    global $INFO;
    if(!$dokuConf['updatecheck']) return;
    if($dokuConf['useacl'] && $INFO['perm'] < AUTH_ADMIN) return;

    $cf = $dokuConf['cachedir'].'/messages.txt';
    $lm = @filemtime($cf);
...
function check(){
  ...
  if(is_writable($dokuConf['changelog'])){
    msg('Changelog is writable',1);
  }else{
    if (@file_exists($dokuConf['changelog'])) {
      msg('Changelog is not writable',-1);
    }
  }

  if (isset($dokuConf['changelog_old']) && @file_exists($dokuConf['changelog_old'])) {
    msg('Old changelog exists', 0);
  }

  if (@file_exists($dokuConf['changelog'].'_failed')) {
    msg('Importing old changelog failed', -1);
  } else if (@file_exists($dokuConf['changelog'].'_importing')) {
    msg('Importing old changelog now.', 0);
  } else if (@file_exists($dokuConf['changelog'].'_import_ok')) {
  ...
  if($dokuConf['allowdebug']){
    msg('Debugging support is enabled. If you don\'t need it you should set $dokuConf[\'allowdebug\'] = 0',-1);
  }else{
...
function dbglog($msg){
  global $dokuConf;
  $file = $dokuConf['cachedir'].'/debug.log';
  $fh = fopen($file,'a');
  if($fh){
