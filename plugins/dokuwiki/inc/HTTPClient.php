...
class DokuHTTPClient extends HTTPClient {
    ...
    function __construct(){
        ...
        $this->proxy_host = $dokuConf['proxy']['host'];
        $this->proxy_port = $dokuConf['proxy']['port'];
        $this->proxy_user = $dokuConf['proxy']['user'];
        $this->proxy_pass = $dokuConf['proxy']['pass'];
        $this->proxy_ssl  = $dokuConf['proxy']['ssl'];
    }
}
    ...
    function sendRequest($url,$data=array(),$method='GET'){
        ...
        if(isset($this->resp_headers['set-cookie'])){
            foreach ((array) $this->resp_headers['set-cookie'] as $c){
                list($key, $value, $foo)  = array_pad(explode('=', $cookie), 3, "");
