...
function p_get_first_heading($id){
  global $dokuConf;
  return $dokuConf['useheading'] ? p_get_metadata($id,'title') : null;
}
...
function p_xhtml_cached_geshi($code, $language) {
  ...
  if (@file_exists($cache) && !$_REQUEST['purge'] &&
    ...
    $geshi->set_link_target($dokuConf['target']['extern']);
