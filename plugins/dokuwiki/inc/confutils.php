...
function confToHash($file,$lower=false) {
  ...
  foreach ( $lines as $line ) {
    ...
    if($lower){
      $dokuConf[strtolower($line[0])] = $line[1];
    }else{
      $dokuConf[$line[0]] = $line[1];
    }
  }
...
function actionOK($action){
  ...
  if(is_null($disabled)){
    ...
    $disabled = explode(',',$dokuConf['disableactions']);
    $disabled = array_map('trim',$disabled);
    if(isset($dokuConf['openregister']) && !$dokuConf['openregister']) $disabled[] = 'register';
    if(isset($dokuConf['resendpasswd']) && !$dokuConf['resendpasswd']) $disabled[] = 'resendpwd';
