...
class cache {
  ...
  function _stats($success) {
    ...
    if (!$dokuConf['allowdebug']) { return $success; }

    if (is_null($stats)) {
      $file = $dokuConf['cachedir'].'/cache_stats.txt';
      $lines = explode("\n",io_readFile($file));
...
class cache_parser extends cache {
  ...
  function _addDependencies() {
    ...
    $this->depends['age'] = isset($this->depends['age']) ? 
                   min($this->depends['age'],$dokuConf['cachetime']) : $dokuConf['cachetime'];
...
class cache_renderer extends cache_parser {
  ...
  function _useCache() {
    ...
    if (isset($this->page)) {
      ...
      if ($this->_time < @filemtime($dokuConf['cachedir'].'/purgefile')) {
