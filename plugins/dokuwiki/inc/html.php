...
function html_btn($name,$id,$akey,$params,$method='get',$tooltip=''){
  ...
  if($dokuConf['userewrite'] == 2){
    $script = DOKU_BASE.DOKU_SCRIPT.'/'.$id;
  }elseif($dokuConf['userewrite']){
    $script = DOKU_BASE.$id;
  }else{
...
function html_draft(){
  ...
  <form id="dw__editform" method="post" action="<?php echo script()?>"
   accept-charset="<?php echo $lang['encoding']?>"><div class="no">
    ...
    <div id="draft__status"><?php echo $lang['draftdate'].' '.date($dokuConf['dformat'],filemtime($INFO['draft']))?></div>
...
function html_search(){
  ...
  if(count($data)){
    ...
    foreach($data as $id){
      print '<li> ';
      print html_wikilink(':'.$id,$dokuConf['useheading']?NULL:$id);
      print '</li> ';
    }
    print '</ul> ';
  ...
  if(count($data)){
    ...
    foreach($data as $id => $cnt){
      ...
      print html_wikilink(':'.$id,$dokuConf['useheading']?NULL:$id,$poswords);
...
function html_locked(){
  ...
  $expire = @date($dokuConf['dformat'], $locktime + $dokuConf['locktime'] );
  $min    = round(($dokuConf['locktime'] - (time() - $locktime) )/60);
...
function html_revisions($first=0){
   ...
   * see html_recent()
   */
  $revisions = getRevisions($ID, $first, $dokuConf['recent']+1);
  if(count($revisions)==0 && $first!=0){
    $first=0;
    $revisions = getRevisions($ID, $first, $dokuConf['recent']+1);;
  }
  $hasNext = false;
  if (count($revisions)>$dokuConf['recent']) {
    $hasNext = true;
    array_pop($revisions); // remove extra log entry
  }

  $date = @date($dokuConf['dformat'],$INFO['lastmod']);
  ...
  foreach($revisions as $rev){
    $date = date($dokuConf['dformat'],$rev);
  ...
  $last = $first + $dokuConf['recent'];
  if ($first > 0) {
    $first -= $dokuConf['recent'];
    if ($first < 0) $first = 0;
...
function html_recent($first=0){
   ...
   */
  $recents = getRecents($first,$dokuConf['recent'] + 1,getNS($ID));
  if(count($recents) == 0 && $first != 0){
    $first=0;
    $recents = getRecents($first,$dokuConf['recent'] + 1,getNS($ID));
  }
  $hasNext = false;
  if (count($recents)>$dokuConf['recent']) {
    $hasNext = true;
  ...
  foreach($recents as $recent){
    $date = date($dokuConf['dformat'],$recent['date']);
    ...
    print html_wikilink(':'.$recent['id'],$dokuConf['useheading']?NULL:$recent['id']);
...
function html_index($ns){
  ...
  $data = array();
  search($data,$dokuConf['datadir'],'search_index',array('ns' => $ns));
...
function html_backlinks(){
  ...
  foreach($data as $blink){
    ...
    print html_wikilink(':'.$blink,$dokuConf['useheading']?NULL:$blink);
    print '</div></li>';
  }
  print '</ul>';
}
...
function html_diff($text='',$intro=true){
  ...
  if($text){
    ...
    $left  = '<a class="wikilink1" href="'.wl($ID).'">'.
              $ID.' '.date($dokuConf['dformat'],@filemtime(wikiFN($ID))).'</a>'.
              $lang['current'];
    $right = $lang['yours'];
  }else{
    if($REV){
...
?>
  ...
  <fieldset style="width: 80%;">
    ...
    <?php if ($dokuConf['profileconfirm']) { ?>
      <br />
      <label class="block">
...
?>
  <div style="width:99%;">
   ...
   <div class="toolbar">
      <div id="draft__status"><?php if(!empty($INFO['draft'])) echo $lang['draftdate'].' '.date($dokuConf['dformat']);?></div>
...
function html_minoredit(){
  ...
  if(!$dokuConf['useacl'] || !$_SERVER['REMOTE_USER']){
    return;
  }
...
function html_admin(){
  ...
  foreach ($pluginlist as $p) {
    ...
    $menu[] = array('plugin' => $p,
                    'prompt' => $obj->getMenuText($dokuConf['lang']),
