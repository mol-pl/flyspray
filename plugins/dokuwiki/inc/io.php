...
function io_sweepNS($id,$basedir='datadir'){
  ...
  while(($id = getNS($id)) !== false){
    $dir = $dokuConf[$basedir].'/'.utf8_encodeFN(str_replace(':','/',$id));
...
function io_saveFile($file,$content,$append=false){
  ...
  if(!$fileexists and !empty($dokuConf['fperm'])) chmod($file, $dokuConf['fperm']);
  io_unlock($file);
  return true;
}
...
function io_lock($file){
  ...
  if($dokuConf['safemodehack']) return;

  $lockDir = $dokuConf['lockdir'].'/'.md5($file);
  @ignore_user_abort(1);
  ...
  do {
    ...
    $locked = @mkdir($lockDir, $dokuConf['dmode']);
    if($locked){
      if(!empty($dokuConf['dperm'])) chmod($lockDir, $dokuConf['dperm']);
      break;
    }
    usleep(50);
  } while ($locked === false);
...
function io_mkdir_p($target){
  ...
  if (io_mkdir_p(substr($target,0,strrpos($target,'/')))){
    if($dokuConf['safemodehack']){
      $dir = preg_replace('/^'.preg_quote(realpath($dokuConf['ftp']['root']),'/').'/','', $target);
      return io_mkdir_ftp($dir);
    }else{
      $ret = @mkdir($target,$dokuConf['dmode']); // crawl back up & create dir tree
      if($ret && $dokuConf['dperm']) chmod($target, $dokuConf['dperm']);
      return $ret;
    }
  }
  return 0;
...
function io_mkdir_ftp($dir){
  ...
  $conn = @ftp_connect($dokuConf['ftp']['host'],$dokuConf['ftp']['port'],10);
  if(!$conn){
    msg("FTP connection failed",-1);
    return false;
  }

  if(!@ftp_login($conn, $dokuConf['ftp']['user'], $dokuConf['ftp']['pass'])){
  ...
  //set permissions
  @ftp_site($conn,sprintf("CHMOD %04o %s",$dokuConf['dmode'],$dir));
...
function io_rename($from,$to){
  ...
  if(!@rename($from,$to)){
    if(@copy($from,$to)){
      if($dokuConf['fperm']) chmod($to, $dokuConf['fperm']);
      @unlink($from);
      return true;
    }
    return false;
