...
function breadcrumbs(){
  ...
  $crumbs[$ID] = $name;
  //reduce size
  while(count($crumbs) > $dokuConf['breadcrumbs']){
    array_shift($crumbs);
  }
  //save to session
  $_SESSION[DOKU_COOKIE]['bc'] = $crumbs;
...
function idfilter($id,$ue=true){
  global $dokuConf;
  if ($dokuConf['useslash'] && $dokuConf['userewrite']){
    $id = strtr($id,':','/');
...
function checkwordblock(){
  ...
  if(!$dokuConf['usewordblock']) return false;
...
function checklock($id){
  ...
  //lockfile expired
  if((time() - filemtime($lock)) > $dokuConf['locktime']){
    @unlink($lock);
    return false;
  }
...
function pageTemplate($data){
  ...
  $tpl = str_replace('@DATE@',date($dokuConf['dformat']),$tpl);
  return $tpl;
}
...
function saveWikiText($id,$text,$summary,$minor=false){
  ...
  */
  ...
  else if ($minor && $dokuConf['useacl'] && $_SERVER['REMOTE_USER']) { $type = 'e'; } //minor edits only for logged in users
  ...
  io_saveFile($dokuConf['cachedir'].'/purgefile',time());
}
...
function notify($id,$who,$rev='',$summary='',$minor=false,$replace=array()){
  ...
  if($who == 'admin'){
    if(empty($dokuConf['notify'])) return; //notify enabled?
    $text = rawLocale('mailtext');
    $to   = $dokuConf['notify'];
    $bcc  = '';
  }elseif($who == 'subscribers'){
    if(!$dokuConf['subscribers']) return; //subscribers enabled?
    if($dokuConf['useacl'] && $_SERVER['REMOTE_USER'] && $minor) return; //skip minors
    $bcc  = subscriber_addresslist($id);
    if(empty($bcc)) return;
    $to   = '';
    $text = rawLocale('subscribermail');
  }elseif($who == 'register'){
    if(empty($dokuConf['registernotify'])) return;
    $text = rawLocale('registermail');
    $to   = $dokuConf['registernotify'];
    $bcc  = '';
  }else{
    return; //just to be safe
  }

  $text = str_replace('@DATE@',date($dokuConf['dformat']),$text);
  ...
  $text = str_replace('@TITLE@',$dokuConf['title'],$text);
  ...
  $subject = '['.$dokuConf['title'].'] '.$subject;

  mail_send($to,$subject,$text,$dokuConf['mailfrom'],'',$bcc);
}
...
/**
 * return an obfuscated email address in line with $dokuConf['mailguard'] setting
 *
...
function subscriber_addresslist($id){
  ...
  if (!$dokuConf['subscribers']) return;
  ...
  if(count($mlist) > 0) {
    foreach ($mlist as $who) {
      ...
      if ($level >= AUTH_READ) {
        if (strcasecmp($info['mail'],$dokuConf['notify']) != 0) {
          if (empty($emails)) {
            $emails = $info['mail'];
          } else {
