...
class Doku_Renderer_xhtml extends Doku_Renderer {
    ...
    function header($text, $level, $pos) {
        ...
        //handle TOC
        if($level >= $dokuConf['toptoclevel'] && $level <= $dokuConf['maxtoclevel']){
            // the TOC is one of our standard ul list arrays ;-)
            $this->toc[] = array( 'hid'   => $hid,
                                  'title' => $text,
                                  'type'  => 'ul',
                                  'level' => $level-$dokuConf['toptoclevel']+1);
        }

        // write the header
		// Nux start - no anchors in headers
		/*
    ...
    function section_edit($start, $end, $level, $name) {
        ...
        if ($start!=-1 && $level<=$dokuConf['maxseclevel']) {
    ...
    */
    ...
    function doublequoteopening() {
        //$this->doc .= "&ldquo;";
		$this->doc .= "&bdquo;";	// Nux fix: pl-quotes
    }
    ...
    */
    ...
    function externallink($url, $name = NULL) {
        ...
        //prepare for formating
        $link['target'] = $dokuConf['target']['extern'];
        $link['style']  = '';
        $link['pre']    = '';
        $link['suf']    = '';
        $link['more']   = '';
        ...
        if($dokuConf['relnofollow']) $link['more'] .= ' rel="nofollow"';
    ...
    */
    function interwikilink($match, $name = NULL, $wikiName, $wikiUri) {
        ...
        $link = array();
        $link['target'] = $dokuConf['target']['interwiki'];
        $link['pre']    = '';
        $link['suf']    = '';
        $link['more']   = '';
        ...
        if(preg_match('#\{(URL|NAME|SCHEME|HOST|PORT|PATH|QUERY)\}#',$url)){
            ...
            //default
            // Nux fix '%2F'->'/'
            $link['url'] = $url.strtr(rawurlencode($wikiUri), array('%2F'=>'/'));
        }
    ...
    function windowssharelink($url, $name = NULL) {
        ...
        global $lang;
        //simple setup
        $link['target'] = $dokuConf['target']['windows'];
        $link['pre']    = '';
        $link['suf']   = '';
        $link['style']  = '';
...
#        }
        ...
        if($dokuConf['mailguard'] == 'visible') $address = rawurlencode($address);
    ...
    function internalmedia ($src, $title=NULL, $align=NULL, $width=NULL,
        ...
        $link['suf']    = '';
        $link['more']   = '';
        $link['target'] = $dokuConf['target']['media'];
        $noLink = false;
    ...
    function rss ($url,$params){
        ...
        if($rc){
            for ($x = $start; $x != $end; $x += $mod) {
                ...
                if($params['date']){
                    $this->doc .= ' ('.$item->get_date($dokuConf['dformat']).')';
                }
                if($params['details']){
    ...
    function _simpleTitle($name){
        ...
        if(!$name) return $this->_simpleTitle($dokuConf['start']);
        return $name;
    }
