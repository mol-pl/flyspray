...
class Doku_LexerParallelRegex {
    ...
    function _getCompoundedRegex() {
		...
		/**
		// debug
		echo "<pre>patterns:\n" . htmlspecialchars(var_export($this->_patterns, true)) . "</pre>";
		echo "<pre>labels:\n" . htmlspecialchars(var_export($this->_labels, true)) . "</pre>";
		/**/
		// Nux: pattern oveflow diagnostics
		if (strlen($this->_regex) > 30000) {
...
class Doku_Lexer {
    ...
    function _invokeParser($content, $is_match, $pos) {
        ...
        if(substr($handler,0,7)=='plugin_'){
            list($handler,$plugin)  = array_pad(explode('_',$handler,2), 2, "");
